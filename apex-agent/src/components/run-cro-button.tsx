...
export function RunCroButton({ defaultTarget }: { defaultTarget: number }) {
  ...
  async function run() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? "CRO review failed");
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "CRO review failed");
    } finally {
      setRunning(false);
    }
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      >
        {running ? "Running executive review…" : "Run CRO review"}
      </button>
